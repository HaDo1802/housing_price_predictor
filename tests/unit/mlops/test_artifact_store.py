...
def test_sync_production_to_local_materializes_production_snapshot(
    ...
    monkeypatch.setattr(
        artifact_store, "resolve_version", lambda model_name, stage=None: "7"
    )
    monkeypatch.setattr(artifact_store, "MlflowClient", lambda: FakeClient())
    monkeypatch.setattr(
        artifact_store.mlflow.sklearn, "load_model", lambda uri: {"uri": uri}
    )
    ...
    monkeypatch.setattr(
        artifact_store, "_download_optional_run_artifact", fake_download
    )
