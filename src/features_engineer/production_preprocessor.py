...
class ProductionPreprocessor:
    ...
    def transform(self, X: pd.DataFrame) -> np.ndarray:
        ...
        if self.verbose:
            logger.info(f"Transformed {len(X)} samples. Output shape: {X_transformed.shape}")
    ...
    def _store_feature_names(self) -> None:
        ...
        if self.categorical_features:
            encoder = self.preprocessor_pipeline.named_transformers_['categorical'].named_steps['encoder']
